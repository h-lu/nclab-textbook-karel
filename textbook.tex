...
\begin{gbox}
\begin{center}
Note that all commands that form the body of the {\tt if} and {\tt else} branches are indented.
The indentation serves the same purpose as in the {\tt repeat} loop. 
\end{center}
\end{gbox}

\subsection{Using the keyword {\tt not}}

Karel can utilize the keyword {\tt not} (negation) in conditions. For illustration, the last 
...
The {\tt gem} sensor allows Karel to detect a gem on the ground. The gem 
must lie in the same square -- the robot cannot see what is one or more 
steps ahead. Consider the situation shown in Fig. \ref{fig:if-111} 
again:
...
repair the pavements. Your program should not throw an error if Karel 
...
\noindent
Is this program OK? Not yet! If the number of
gems in Karel's bag is less than the number of missing tiles
...
\noindent
Good job! Fig. \ref{fig:empty-112} shows the pavement after the program 
is execured. Before running the program, we inserted 15 gems into Karel's
bag in Designer.
...
\vspace{-0mm}
\caption{After running the program, the pavement is repaired!}
\label{fig:empty-112}
%\vspace{-1cm}
\end{center}
\end{figure}
%\newpage
\noindent
Try to return to the maze from Fig. \ref{fig:empty-111} and run this program with 
just 10 gems in the robot's bag! What is supposed to happen?
\newpage

\subsection{Gambling}

Let us return to Fig. \ref{fig:if-111b}:
...
\noindent
If the program stops with an error message, we say that it {\em crashed} (even 
though Karel did not crash).\\

\begin{gbox}
\begin{center}
Do not take chances. Try to foresee and avoid situations\\ 
when your program might crash. 
\end{center}
\end{gbox}
\vspace{6mm}
...
by using the {\tt gem} sensor before trying to pick up the gem:\\
...
\noindent
Also the command {\tt put} can cause an error when the robot's
bag is empty. It is a good practice to always combine the {\tt put} 
command with using the {\tt empty} sensor:\\
...
\begin{gbox}
\begin{center}
It is always a good idea to check the corresponding sensor prior to using 
commands {\tt go}, {\tt get}, or {\tt put}. Checking a sensor does not make 
your program slower.
\end{center}
\end{gbox}
\vspace{6mm}
...
West, etc. The {\tt home} sensor helps the robot to make sure that he 
...
The {\em conditional loop} is another important concept that is present in all procedural 
...
when Karel needs to walk straight to the closest wall. Remember that he only can detect 
a wall that is immediately in front of him, not a wall that is one or more steps ahead. 

In most languages (including Karel), the conditional loop is represented via the 
keyword {\tt while}. In others, one can find other keywords such as {\tt do - while}, 
{\tt do - until}, {\tt repeat - until} etc.
...
Consider the situation shown in Fig. \ref{fig:while-111}. Karel 
stands at a random position in a maze that contains no walls.
There only is one gem, located at an unknown position somewhere along 
the exterior wall. Let us write a program for the robot to find and collect 
the gem!
...
\vspace{-0mm}
\caption{There only is one gem, located somewhere at the exterior wall.}
\label{fig:while-111}
\vspace{-6mm}
\end{center}
\end{figure}
...
\subsection{Think like a robot!}
Next let us solve the problem shown in Fig. \ref{fig:while-114}. Karel knows that 
his home is straight ahead, that there are no walls to crash into,  
and that there are gems on the ground that he needs to collect. 
\newpage
...
\vspace{-0mm}
\caption{Karel needs to walk home and collect all gems on the way.}
\label{fig:while-114}
\vspace{-6mm}
\end{center}
\end{figure}
\noindent
This could be done using the following program:\\
...
\noindent
Clearly, the author was thinking like an observer who knows everything about the maze, 
including the exact positions of all gems. Incorporating this knowledge into the program 
was wrong, since it was not part of the task description. As a result, the above program 
is extremely rigid -- it will only work for this particular maze. A much better approach 
is to think like the robot:\\

\begin{gbox}
\begin{center}
Cover the maze (the robot cannot see it either). Keep in mind only two things:\\
the task at hand, and the robot's abilities. 
\end{center}
\end{gbox}
\vspace{6mm}

\noindent
This is the way to become a great thinker and programmer. 
It leads to a completely different program:\\
...
\noindent
The program is not only much shorter and more elegant, but also more universal. 
It will work for any maze where the home is straight ahead of the robot, with no walls 
between the robot and his home. One such maze is shown in Fig. \ref{fig:while-115}.
\newpage
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=14cm]{img/while-115.png}
\vspace{-0mm}
\caption{The latter program now works for many different mazes.}
\label{fig:while-115}
\vspace{-6mm}
\end{center}
\end{figure}
...
When solving a more complex task, it is a good idea to always check whether 
it contains simpler problems that could be solved first. Once these smaller 
problems are solved, and custom commands ({\em subprograms}) are created for 
them, then the original task does not appear that complex anymore. 

\subsection{Objectives} 
 
\begin{itemize}
\item Learn to split complex tasks into smaller ones. 
\item Learn that creating new commands makes programs simpler.
\end{itemize}
...
in various situations, it is a good idea to define a new command {\tt turnback}:\\
...
\noindent
Note that the body of a new command needs to be indented 
analogously to the body of loops and conditions. \\

\noindent
The following paragraph illustrates how more complex tasks can be 
split into smaller ones, and how custom commands can be used. .
...
Clearly, this problem is more complex than the problems that we 
...
the {\tt turnback} command that we know from before:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} New command to turn back:}
\PY{k}{def} turnback
    \PY{k}{repeat} 2
        \PY{n+nf}{left}
\end{Verbatim}
...
\noindent
Almost there! The last new command we need is to get to the next 
...
\noindent
The main program includes just the command {\tt arcade}:\\
...
\noindent
Fig. \ref{fig:newcomm-112} shows the arcade after the program has finished:

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=14cm]{img/newcomm-112.png}
\vspace{-0mm}
\caption{Arcade after our program has finished.}
...
home of his parents to experience life on his own. Therefore, his home square will not be 
...
work with variables and lists, employ functions that return values, use complex 
logical operations, make random decisions, and more. Do not forget to switch to 
Programming Mode 3 in the menu. A compact overview of new functionality in 
Programming Mode 3 can be found in Section \ref{sec:newfunc3}.
...
\item Learn to define and use functions that return values. 
...
\noindent
Say that your name is "Melissa". When you were about two years old, you 
saved your own name in your memory. Using computer language, this can 
...
\noindent
Of course, for someone this variable will have the value {\tt True} and for someone else 
it will be {\tt False}. Logical variables and operations will be discussed in more detail in Section 
\ref{sec:logic}. 
...
\noindent
Move Karel to other parts of the maze and run the program again, to make yourself 
familiar with how the GPS device works!\\
...
In Programming Mode 3 we can use the keyword {\tt return} in the body of
a command to return a value. Such commands are called {\em functions}. 
They are also defined using the keyword {\tt def}. For example, with the following function
{\tt count\_steps\_to\_wall} the robot will walk straight towards the closest wall and 
return the number of steps he has done to reach it:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{k}{def} count_steps_to_wall
    n = 0
    \PY{k}{while} \PY{n+nb+bp}{not} \PY{n+nb+bp}{wall}
        \PY{n+nf}{go}
        \PY{n+nf}{inc}(n)
...
\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{n}{num} \PY{o}{=} \PY{n}{count\PYZus{}steps\PYZus{}to\PYZus{}wall}
\PY{k}{print} \PY{l+s}{"}\PY{l+s}{I reached a wall in}\PY{l+s}{"}\PY{p}{,} \PY{n}{num}, \PY{l+s}{"}\PY{l+s}{steps!\PY{l+s}{"}}
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
Karel does not know mathematical symbols '+' and '-'. But for 
counting purposes, he can increase and decrease the value of an 
integer variable by one using the functions {\tt inc()} and 
{\tt dec()}, respectively. He can also increase and decrease the
value of a variable by more than one via {\tt inc(n, num)} and 
{\tt dec(n, num)}, where {\tt n} is the name of the variable and 
{\tt num} is an integer number.
...
\PY{k}{while} \PY{n+nb+bp}{not} \PY{n+nb+bp}{wall}
    \PY{n+nf}{go}
    \PY{n+nf}{inc}(a)
\PY{n+nf}{print} \PY{l+s}{"I made"}, a, \PY{l+s}{"steps before reaching a wall."}
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
\begin{ybox}
\begin{verbatim}
I made 7 steps before reaching a wall.
\end{verbatim}
\end{ybox}
\vspace{6mm}
...
\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{k}{print} \PY{s+1}{"Robot's name is"}, robots_name
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
version of "Karel"),
and store his original name in the variable {\tt robots\_name\_orig}. 
This is done as follows:\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
robots_name_orig = robots_name
robots_name = \PY{s+1}{"Carlos"}
\end{Verbatim}
\end{bbox}

\subsection[\ \ Local and global variables]{Local and global variables}\label{subsec:karellocvar}

A variable that is created within a function is {\em local to that function}, 
meaning that it can be used in the function only. If we attempt to use it 
outside of that function, an error is thrown. The following code creates
a local variable {\tt a} within a function called {\tt myfunction}:\\
...
\noindent
On the other hand, variables created within the main program 
are {\em global}. Global variables can be used in functions. 
For illustration, let us adjust the above program to:\\
...
\begin{gbox}
\begin{center}
Using local variables is extremely helpful as it keeps 
the code well organized.\\
Global variables should be avoided, or at least kept to a minimum.
\end{center}
\end{gbox}
\vspace{6mm}

\noindent
More about local and global 
variables is said in the NCLab Python textbook which is available 
at {\tt http://femhub.com/textbook-python}. 
...
\item Learn basic operations with lists.
\item Learn to work with indices.
\end{itemize}
Lists are very useful data structures that can be used to store multiple 
integer values, GPS coordinates, logical values, and/or text strings at the same time. 
Values of different types can be combined and lists can even contain other lists.
For example, a list can store the robot's path, positions of all gems in the maze, 
positions and shapes of obstacles that the robot encounters, and much more.
Objects in a list are ordered and they can be added to the end of a list, 
accessed by their index, and deleted from any position of the list. 

\subsection[\ \ Compatibility with Python]{Compatibility with Python} 

Karel lists are the same as lists in the Python programming language, so all you learn here 
is directly transferable to Python. The Python language provides additional, more advanced 
functionality for lists which is not present in Karel.

\subsection[\ \ Creating a list]{Creating a list}

\noindent
We use square brackets when creating lists. An empty list {\tt U} is created as follows:\\
...
\subsection[\ \ Accessing list items by their indices]{Accessing list items by their indices}

\noindent
List items can be accessed and either printed, assigned 
to other variables, or used in some other way, via their indices. 
Indices always start from zero. In other words, {\tt L[0]} is the 
first item in the list {\tt L}, {\tt L[1]} is the second item, etc. 
Working with indices can be illustrated using the following simple code\\
...
In Karel, lists can be parsed via the {\tt for} command that is the same as the 
corresponding command in Python.
For illustration, the following code defines a list {\tt M} consisting of four numbers 
{\tt 1, 2, 3, 4} and prints each of them, increased by two: \\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
M = [1, 3, 5, 7]
\PY{k}{for} n \PY{k}{in} M
    \PY{k}{print} \PY{n+nf}{inc}(n, 2)
\end{Verbatim}
\end{bbox}
...
\noindent
The following 
sample program tells the robot to walk straight to the 
nearest wall and store his path in the form of {\tt [gpsx, gpsy]}
pairs in a list {\tt L}:\\
...
\noindent
When the robot's initial position is as shown in Fig. 
\ref{fig:list-1},
\newpage
...
\subsection[\ \ Storing positions of gems in a list]{Storing positions of gems in a list}

\noindent
In this example, gems are distributed randomly along the perimeter of the maze.
The task for the robot is to walk around the maze boundary, and record into a list 
the positions and counts of all gems that he finds. Each list entry should have 
the form {\tt [gpsx, gpsy, number]}. At the end, he should print the list.
\newpage

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=14cm]{img/lists-1b.png}
\vspace{-0mm}
\caption{Gems are distributed randomly along maze perimeter.}
%\vspace{-1cm}
\label{fig:list-1b}
\end{center}
\end{figure}
...
\noindent
One possible program to do this looks as follows. First we write 
a function {\tt countgems} that will count the number of gems in a square
...
\PY{k}{def} countgems
  num = 0
  \PY{c}{\PYZsh{} Collect all gems:}
  \PY{k}{while} gem
    \PY{k}{get}
    \PY{n+nf}{inc}(num)
  \PY{c}{\PYZsh{} Put the gems back:}
  \PY{k}{repeat} num
    \PY{k}{put}
  \PY{k}{return} num
\end{Verbatim}
...
\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} Create an empty list:}
L = []

\PY{c}{\PYZsh{} Walk along 4 walls:}
\PY{k}{repeat} 4
  \PY{k}{while} \PY{n+nf}{not wall}
    \PY{k}{go}
    \PY{k}{if} \PY{n+nf}{gem}
      L.append([\PY{n+nf}{gpsx}, \PY{n+nf}{gpsy}, countgems])
  \PY{k}{left}

\PY{c}{\PYZsh{} Print the list:}
\PY{k}{print} \PY{s+1}{"Here is the report:"}
\PY{k}{print} L
\end{Verbatim}
\end{bbox}
\vspace{6mm}
...
\noindent
For the maze shown in Fig. \ref{fig:list-1b}, the output of the program is\\

\begin{ybox}
\begin{Verbatim}[commandchars=\\\{\}]
Here is the report:
[[14, 1, 1], [14, 3, 3], [14, 7, 6], [14, 8, 2], [14, 11, 9], 
[11, 11, 1], [10, 11, 2], [6, 11, 5], [3, 11, 2], [0, 11, 4], 
[0, 7, 2], [0, 5, 3], [0, 2, 1], [0, 0, 2], [1, 0, 1], 
[5, 0, 2], [7, 0, 1], [12, 0, 6]]
\end{Verbatim}
\end{ybox}

\subsection[\ \ Lists contained in other lists]{Lists contained in other lists}

Notice that the result of the last program is a list {\tt L} that contains other (embedded) lists.
In other programming languages, items of embedded lists are accessed via multiple indices. 
This is not allowed in Karel for the sake of simplicity, but still the same can be done 
easily. Look, for example, at the following program that goes through the list {\tt L} 
and creates another list {\tt L2} that only contains the counts of gems (their 
coordinates are left out):\\

\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
L2 = []
\PY{k}{for} x \PY{k}{in} L
  L2.\PY{k}{append}(x[2])
\PY{k}{print} \PY{s+1}{"Skipping the coordinates:"}
\PY{k}{print} L2
\end{Verbatim}
\end{bbox}
...
\section{Elementary and Advanced Logic} \label{sec:logic} 
...
As we already know, logical expressions are expressions that are either {\tt True} or 
{\tt False}. We say that {\tt True} or {\tt False} is their {\em value}. Here are some 
real-life examples, try to answer them with {\tt True} or {\tt False}:

\begin{itemize}
\item "I am 12 years old."
\item "My dad is a teacher."
\item "My school is a STEM Academy."
\end{itemize}
...
\noindent
Given the situation in Fig. \ref{fig:logic1}, the output of this code is:\\
...
\subsection[\ \ More complex logical expressions]{Complex logical expressions}

In programming as well as in real life we often deal with logical expressions that are 
more complex. Often we use two or more simple logical expressions in one sentence, 
...
Michael joins me." includes three simple logical expressions. Let's call 
them for brevity\\

\noindent
{\tt A} = "I will go skiing on Saturday."\\
{\tt B} = "The weather is good."\\
{\tt C} = "Michael joins me."\\
...
\vspace{4mm}
\noindent
We love this kind of brevity in programming because it makes the code short. 
...
\begin{itemize}
\item Learn how to make random decisions using the {\tt rand} command.
\item Learn to simulate random processes.
\end{itemize}
...
As a first example, here is a nice short program that makes Karel walk randomly 
...
\begin{bbox}
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c}{\PYZsh{} Return to upper right corner and face South:}
\PY{k}{def} GoCorner
  \PY{k}{while} \PY{n+nb+bp}{not} \PY{n+nb+bp}{north}
    \PY{n+nf}{left}
  \PY{n+nf}{right}
...
gems along the South wall approximates the binomial distribution. 
...
\noindent
Credit for this example goes to Ed Keppelmann from UNR. The example 
can be downloaded from NCLab's public database.
...
\subsection[\ \ Programming Mode 2 (Sections \ref{sec:cond} -- \ref{sec:newcom})]{Programming Mode 2 (Sections \ref{sec:cond} -- \ref{sec:newcom})}
...
\subsection[\ \ Programming Mode 3 (Sections \ref{sec:var} - \ref{sec:recursion})]{Programming Mode 3 (Sections \ref{sec:var} - \ref{sec:recursion})}
...
engineering, and other areas. NCLab provides a Python programming 
module which is suitable for learning, and a free textbook that can be found at 
{\tt http://femhub.com/textbook-python}.\\
